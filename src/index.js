...
var CdpPage = class _CdpPage extends Page {
  ...
  #onLogEntryAdded(event) {
    ...
    if (source2 !== "worker") {
      this.emit("console", new ConsoleMessage(convertConsoleMessageLevel(level2), text, [], [{ url, lineNumber }]));
    }
  }
  mainFrame() {
...
var WorkersWebSocketTransport = class _WorkersWebSocketTransport {
  ...
  onclose;
  sessionId;
  static async create(endpoint, sessionId, browser) {
    const path = browser ? `${FAKE_HOST}/v1/devtools/browser?browser=${browser}` : `${FAKE_HOST}/v1/devtools/browser/${sessionId}`;
...
var PuppeteerWorkers = class extends Puppeteer {
  ...
  async launch(endpoint, options) {
    if (options?.browser) {
      return await this.connect(endpoint, void 0, options);
    }
  ...
  async connect(endpoint, sessionId, options) {
    const browser = sessionId ? void 0 : options?.browser;
    try {
      if (!sessionId && !browser) {
        return await super.connect(endpoint);
      }
      const connectionTransport = await WorkersWebSocketTransport.create(endpoint, sessionId, browser);
  ...
  async acquire(endpoint, options) {
    ...
    const acquireUrl = `${FAKE_HOST2}/v1/devtools/browser?${searchParams.toString()}`;
...
// src/index.js
var LOCATION_ID = "zyhFEkFNE1Eo2O7I8nOP";
var GHL_BASE = "https://app.gohighlevel.com";
...
__name(controlPage, "controlPage");
var index_default = {
  async fetch(request, env) {
    ...
    if (request.headers.get("x-admin-key") !== env.BROWSER_ADMIN_KEY) {
...
var BrowserManager = class extends DurableObject {
  static {
    __name(this, "BrowserManager");
  }
  ...
  async scheduleHeartbeat() {
    try {
      await this.storage.setAlarm(Date.now() + 24e4);
    } catch {
    }
  }
  async captureAuthState(browser = this.browser) {
    if (!browser || !browser.isConnected()) {
      return false;
    }
    const pages = await browser.pages();
    const page = pages.find((item) => item.url().includes("app.gohighlevel.com"));
    if (!page) {
      return false;
    }
    try {
      ...
      await this.storage.put("highLevelAuthState", {
        cookies,
        localStorage,
  ...
  async restoreAuthState(browser) {
    const saved = await this.storage.get("highLevelAuthState");
    if (!saved) {
      return false;
    }
    const pages = await browser.pages();
    ...
    try {
      ...
      await page.goto(`${GHL_BASE}/v2/location/${LOCATION_ID}/dashboard`, {
        waitUntil: "domcontentloaded",
        timeout: 3e4
      });
      await sleep(1500);
  ...
  async launchRestoredBrowser() {
    this.browser = await launch(this.env.BROWSER, {
      keep_alive: 6e5
    });
    await this.storage.put("loginSessionId", this.browser.sessionId());
    await this.restoreAuthState(this.browser);
    ...
    try {
      ...
      if (browser && browser.isConnected()) {
        ...
        if (page) {
          ...
          await this.captureAuthState(browser);
        }
      }
    } catch {
    } finally {
  ...
  async connectToBrowser() {
    ...
    const sessionId = await this.storage.get(
      "loginSessionId"
    );
    if (!sessionId) {
      return await this.launchRestoredBrowser();
    }
    try {
      ...
      await this.captureAuthState(this.browser);
      await this.scheduleHeartbeat();
      return this.browser;
    } catch {
      return await this.launchRestoredBrowser();
  ...
  async getBuilderContext() {
    ...
    const page = await this.findBuilderPage(browser);
    if (!page) {
      throw new Error(
        "No verified HighLevel page builder is open."
      );
    }
    const frame = await this.getBuilderFrame(page);
    if (!frame) {
      throw new Error(
  ...
  async findExactClickPoint(page, text) {
    return page.evaluate((target) => {
      const clean = /* @__PURE__ */ __name((value) => String(value || "").replace(/\\s+/g, " ").trim(), "clean");
      ...
      const candidates = Array.from(
        ...
        return rect2.width > 0 && rect2.height > 0 && clean(
          el.innerText || el.textContent || el.getAttribute(
            "aria-label"
          )
        ).toLowerCase() === wanted;
      });
  ...
  async navigateToFunnels(page) {
    const url = `${GHL_BASE}/v2/location/${LOCATION_ID}/funnels-websites/funnels`;
    if (page.url() !== url) {
      await page.goto(
        url,
        {
  ...
  async findFunnelClickPoint(page, funnelName) {
    return page.evaluate((name) => {
      ...
      const target = Array.from(
        ...
        return clean(
          el.innerText || el.textContent
        ).toLowerCase() === wanted && getComputedStyle(el).cursor === "pointer" && rect2.width > 20 && rect2.height > 10;
      });
      if (!target) {
        return null;
      }
  ...
  async openStoredFunnel(page, funnelName) {
    await this.navigateToFunnels(page);
    const before = page.url();
    const point = await this.findFunnelClickPoint(
      page,
      funnelName
    );
    if (!point) {
  ...
  async selectStepOverview(page, stepName) {
    const current = await this.snapshot(page);
    const lower = current.body.toLowerCase();
    if (lower.includes(
      stepName.toLowerCase()
    ) && lower.includes("overview") && lower.includes("publishing") && lower.includes("edit")) {
      await this.storage.put(
        "currentStepName",
        stepName
      );
    ...
    const point = await this.findExactClickPoint(
      page,
      stepName
    );
    if (!point) {
    ...
    if (!after.body.toLowerCase().includes(
      stepName.toLowerCase()
    )) {
      throw new Error(
  ...
  async inspectFunnels() {
    ...
    const pages = await this.highLevelPages(browser);
    const page = pages[0] || await browser.newPage();
    await this.navigateToFunnels(page);
    const result = await this.snapshot(page);
    return json({
  ...
  async openFunnel(request) {
    ...
    const pages = await this.highLevelPages(browser);
    const page = pages[0] || await browser.newPage();
    const result = await this.openStoredFunnel(
      page,
      name
    );
    return json({
  ...
  async listFunnelSteps() {
    ...
    const result = await this.snapshot(pages[0]);
    return json({
      status: "funnel-steps-inspection-success",
      readOnly: true,
      currentFunnel: await this.storage.get(
        "currentFunnelName"
      ) || null,
      result
    });
  ...
  async inspectFunnelStep(request) {
    ...
    const page = pages[0];
    const result = await this.selectStepOverview(
      page,
      name
    );
    return json({
      status: "funnel-step-inspection-success",
      stepName: name,
      readOnly: true,
      result
    });
  ...
  async openPageBuilder() {
    ...
    const existing = await this.findBuilderPage(
      browser
    );
    if (existing) {
      const frame2 = await this.getBuilderFrame(
        existing
      );
      return json({
        status: "page-builder-open",
        verified: Boolean(frame2),
        alreadyOpen: true,
        url: existing.url()
      });
    }
    ...
    const page = pages[0];
    const funnel = await this.storage.get(
      "currentFunnelName"
    );
    const step = await this.storage.get(
      "currentStepName"
    );
    if (!funnel || !step) {
      return json({
        status: "funnel-or-step-not-selected"
      }, 409);
    }
    const snap = await this.snapshot(page);
    if (!snap.body.includes(step) || !snap.body.includes("Edit")) {
    ...
    for (let i = 0; i < 40; i++) {
      await sleep(750);
      builderPage = await this.findBuilderPage(
        browser
      );
      if (builderPage) {
        break;
    ...
    if (!builderPage) {
      return json({
        status: "page-builder-not-verified"
      }, 409);
    }
    const frame = await this.getBuilderFrame(
      builderPage
    );
    if (!frame) {
      return json({
        status: "builder-frame-not-found"
      }, 409);
    }
    await this.storage.put(
      "builderUrl",
  ...
  async inspectPageBuilder() {
    try {
      ...
      const body = await frame.evaluate(
        () => (document.body?.innerText || "").slice(0, 15e3)
      );
      return json({
        status: "page-builder-inspection-success",
        verified: true,
        readOnly: true,
        builder: {
          url: page.url(),
  ...
  async inspectBuilderElements() {
    try {
      ...
      const result = await frame.evaluate(() => {
        const clean = /* @__PURE__ */ __name((value) => String(value || "").replace(/\\s+/g, " ").trim(), "clean");
        const visible = /* @__PURE__ */ __name((el) => {
      ...
      return json({
        status: "builder-elements-inspection-success",
        verified: true,
        readOnly: true,
        builderUrl: page.url(),
        frameUrl: frame.url(),
      ...
      return json({
        status: "builder-elements-inspection-failed",
        message: error.message
      }, 409);
    }
  }
  ...
  async editBuilderText(request) {
    ...
    if (!target) {
      return json({
        status: "builder-edit-target-not-found",
        selector
      }, 404);
    }
    const beforeText = await frame.$eval(
      selector,
      (el) => String(
        el.innerText || el.textContent || ""
      ).replace(/\\s+/g, " ").trim()
    );
    if (expectedText && beforeText !== this.clean(expectedText)) {
      return json({
        status: "builder-edit-safety-check-failed",
        message: "The current text does not match expectedText, so no edit was made.",
        selector,
        expectedText: this.clean(
          expectedText
        ),
        actualText: beforeText
      }, 409);
    }
    await frame.$eval(
      selector,
    ...
    await sleep(500);
    const editableInfo = await frame.evaluate(
      (selector2) => {
        ...
        const wrapper = selected.closest(
          ".element"
        ...
        return {
          found: true,
          contenteditable: editor.getAttribute(
            "contenteditable"
          ),
    ...
    if (!editableInfo.found) {
      return json({
        status: "builder-inline-editor-not-found",
        message: "The target was found, but HighLevel did not expose its TipTap editor after clicking. No text was changed.",
        selector,
        beforeText
      }, 409);
    }
    ...
    await sleep(600);
    const afterText = await frame.$eval(
      selector,
      (el) => String(
        el.innerText || el.textContent || ""
      ).replace(/\\s+/g, " ").trim()
    );
    const expectedAfter = this.clean(newText);
    if (afterText !== expectedAfter) {
      return json({
        status: "builder-text-edit-not-verified",
        message: "HighLevel received the edit interaction, but the resulting visible text did not exactly match the requested text.",
        selector,
        beforeText,
        requestedText: expectedAfter,
        actualText: afterText
      }, 409);
    }
    await this.storage.put(
  ...
  async inspectBuilderLayout() {
    try {
      ...
      const result = await frame.evaluate(() => {
        ...
        const selectorFor = /* @__PURE__ */ __name((el) => {
  ...
  async inspectBuilderElement(request) {
    ...
    try {
      const { page, frame } = await this.getBuilderContext();
      const result = await frame.evaluate((selector2) => {
        ...
        return {
          tag: el.tagName.toLowerCase(),
          id: el.id || "",
          text: String(el.innerText || el.textContent || "").replace(/\s+/g, " ").trim().slice(0, 1e3),
          classes: Array.from(el.classList || []),
          outerHTML: el.outerHTML.slice(0, 12e3),
          rect: { x: Math.round(r.x), y: Math.round(r.y), width: Math.round(r.width), height: Math.round(r.height) },
          computedStyle: {
            color: s.color,
            backgroundColor: s.backgroundColor,
  ...
  async styleBuilderElement(request) {
    ...
    const before = await frame.evaluate(({ selector: selector2, expectedText: expectedText2 }) => {
      const el = document.querySelector(selector2);
      if (!el) return { found: false };
      const clean = /* @__PURE__ */ __name((v) => String(v || "").replace(/\s+/g, " ").trim(), "clean");
      const text = clean(el.innerText || el.textContent || "");
      return { found: true, text, expectedMatches: !expectedText2 || text === clean(expectedText2) };
    }, { selector, expectedText });
    if (!before.found) return json({ status: "builder-style-target-not-found", selector }, 404);
    if (!before.expectedMatches) return json({
      status: "builder-style-safety-check-failed",
      message: "Current text does not match expectedText; no style change was made.",
      selector,
      expectedText: this.clean(expectedText),
      actualText: before.text
    }, 409);
    const result = await frame.evaluate(({ selector: selector2, styles: styles2 }) => {
      const el = document.querySelector(selector2);
      if (!el) return null;
      const before2 = {};
      for (const key of Object.keys(styles2)) before2[key] = getComputedStyle(el)[key] || el.style[key] || "";
      for (const [key, value] of Object.entries(styles2)) el.style[key] = String(value);
      el.dispatchEvent(new Event("input", { bubbles: true }));
      el.dispatchEvent(new Event("change", { bubbles: true }));
      const after = {};
      for (const key of Object.keys(styles2)) after[key] = getComputedStyle(el)[key] || el.style[key] || "";
      return { before: before2, requested: styles2, after, inlineStyle: el.getAttribute("style") || "" };
    }, { selector, styles });
    ...
    const verification = Object.entries(styles).map(([property, requested]) => ({
      ...
      actual: String(result.after[property] || "")
    }));
  ...
  async builderLastSavedText(frame) {
    try {
  ...
  async saveBuilder() {
    ...
    const selector = "#pg-funnel-builder__btn--save";
    const button = await frame.$(
      selector
    );
    if (!button) {
      return json({
        status: "builder-save-button-not-found",
        selector
      }, 404);
    }
    const before = await this.builderLastSavedText(
      frame
    );
    await button.click();
    await sleep(2500);
    const after = await this.builderLastSavedText(
      frame
    );
    const saveButtonExists = Boolean(
      await frame.$(
        selector
      )
    );
    if (!saveButtonExists) {
      return json({
        status: "builder-save-not-verified",
        message: "Save was clicked but the builder state could not be verified."
      }, 409);
    }
    await this.storage.put(
      "builderHasUnsavedChanges",
    ...
    return json({
      status: "builder-saved",
      verified: true,
      published: false,
      previousSaveLabel: before || null,
      currentSaveLabel: after || null
    });
  }
  async publishBuilder(request) {
    ...
    if (args.confirm !== true) {
      return json({
        status: "publish-confirmation-required",
        message: "Publishing changes the live funnel. Call again with confirm=true."
      }, 409);
    }
    const {
      frame
    } = await this.getBuilderContext();
    const selector = "#pg-funnel-builder__btn--publish";
    const publishButton = await frame.$(
      selector
    );
    if (!publishButton) {
      return json({
        status: "builder-publish-button-not-found",
        selector
      }, 404);
    }
    ...
    const confirmation = await frame.evaluate(() => {
      const clean = /* @__PURE__ */ __name((value) => String(value || "").replace(/\\s+/g, " ").trim().toLowerCase(), "clean");
      ...
      const candidates = buttons.filter(visible).map((el, index) => ({
        index,
        text: clean(
          el.innerText || el.textContent || el.getAttribute(
            "aria-label"
      ...
      const match = candidates.find(
        (item) => item.text === "publish" || item.text === "confirm" || item.text === "publish changes"
      );
      return match || null;
    });
    if (confirmation) {
      ...
      if (buttons[confirmation.index]) {
        await buttons[confirmation.index].click();
        await sleep(2500);
      }
    }
    const result = await frame.evaluate(() => {
      ...
      return {
        hasLiveIndicator: /•\\s*Live/i.test(
          body
        ) || /\\bLive\\b/i.test(
          body
        ),
    ...
    if (!result.hasPublishButton) {
      return json({
        status: "builder-publish-not-verified",
        message: "Publish was clicked but the builder could not be verified afterward.",
        result
      }, 409);
    }
    await this.storage.put(
    ...
    return json({
      status: "builder-published",
      verified: true,
      liveIndicator: result.hasLiveIndicator
    });
  }
  async status() {
    const sessionId = await this.storage.get(
    ...
    return json({
      ...
      currentStep: await this.storage.get(
        "currentStepName"
      ) || null,
      builderUrl: await this.storage.get(
        "builderUrl"
      ) || null,
      builderOpen,
      builderHasUnsavedChanges: await this.storage.get(
        "builderHasUnsavedChanges"
      ) || false,
      builderLastSavedAt: await this.storage.get(
        "builderLastSavedAt"
      ) || null,
      builderLastPublishedAt: await this.storage.get(
        "builderLastPublishedAt"
      ) || null
    });
  }
...
@cloudflare/puppeteer/lib/esm/puppeteer/cloudflare/protocol.js:
